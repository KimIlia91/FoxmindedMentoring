	...
	public class OrderController : Controller
	{
        ...
        public IActionResult Details(int orderId)
        {
			OrderVM = new OrderVM()
			{
				...
				OrderDetails = _unitOfWork.OrderDetail.GetAll(u => u.OrderId == orderId, includeProperty: "Product"),
			};
		...
		public IActionResult DetailsPayNow()
		{
			OrderVM.OrderHeader = _unitOfWork.OrderHeader.GetFirstOrDefault(u => 
				u.Id == OrderVM.OrderHeader.Id, includeProperty: "ApplicationUser");
			OrderVM.OrderDetails = _unitOfWork.OrderDetail.GetAll(u => 
				u.OrderId == OrderVM.OrderHeader.Id, includeProperty: "Product");
			//stripe settings
			...
			_unitOfWork.OrderHeader.UpdateStripeOrderPaymentId(OrderVM.OrderHeader.Id, session.Id, session.PaymentIntentId);
			_unitOfWork.Save();
		...
		{
			// не нужно добавлять в таблицу OrderHeader и вызывать метод EF core делает сам за счет Трекинга
		...
		{
			var orderHeader = _unitOfWork.OrderHeader.GetFirstOrDefault(u => u.Id == OrderVM.OrderHeader.Id, tracked: false);
			if(orderHeader.PaymentStatus is SD.PaymentStatusApproved)
			{
			...
			_unitOfWork.Save();
			TempData["success"] = "Order cansalled successfully!";
			return RedirectToAction("Details", "Order", new { orderId = OrderVM.OrderHeader.Id });
		}
